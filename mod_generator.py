...
def ensure_template(template_dir: Path) -> None:
    ...
    if missing:
        raise FileNotFoundError(
            ...
            + ", ".join(missing)
            + "\nPlace Forge MDK files in the template folder."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--ai-url",
        default=os.getenv("OLLAMAFREEAPI_URL") or os.getenv("OPENAI_API_URL") or "https://api.openai.com/v1/chat/completions",
        help="ollamafreeapi/OpenAI-compatible chat endpoint",
    )
    p.add_argument(
        "--ai-key",
        default=os.getenv("OLLAMAFREEAPI_KEY") or os.getenv("OPENAI_API_KEY") or "",
        help="API key/token",
    )
    p.add_argument("--ai-model", default=os.getenv("OLLAMAFREEAPI_MODEL") or os.getenv("OPENAI_MODEL") or "gpt-4o-mini", help="Model name")
    return p.parse_args()
...
def main() -> int:
    ...
    args = parse_args()
    if not args.ai_url:
        print("error: provide --ai-url, set OLLAMAFREEAPI_URL/OPENAI_API_URL, or add it to .env", file=sys.stderr)
        return 2
    if not args.ai_key:
        print("error: provide --ai-key, set OLLAMAFREEAPI_KEY/OPENAI_API_KEY, or add it to .env", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"error: AI HTTP {exc.code} {exc.reason}\n{detail}", file=sys.stderr)
        return 1
